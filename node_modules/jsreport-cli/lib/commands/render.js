  ...
  return new Promise(function (resolve, reject) {
    ...
    if (remote) {
      return jsreportClient(remote.url, remote.user, remote.password).render(request, function (err, response) {
        ...
        saveResponse(response, response.headers, output, meta, resolve, reject)
      })
    }
    ...
    return resolve(
      jsreportInstance.render(request).then(function (out) {
        return new Promise(function (resolve, reject) {
          saveResponse(out.stream, out.headers, output, meta, resolve, reject)
        })
      }).catch(function (err) {
...
function saveResponse (stream, headers, output, meta, onFinish, onError) {
  var outputStream = writeFileFromStream(stream, output)
  ...
  listenOutputStream(outputStream, function () {
    ...
    fs.writeFile(meta, JSON.stringify(responseHeadersToMetaPOCO(headers)), 'utf8', function (err) {
      if (err) {
        return onError(onRenderingError(err))
      }
...
function responseHeadersToMetaPOCO (headers) {
  ...
  for (var property in headers) {
    if (headers.hasOwnProperty(property)) {
      var val = headers[property]
      var p = property.replace(/-.{1}/g, function (s) {
