...
Sheet = (function() {
  ...
  Sheet.prototype.raw = function(xml) {
    this._raw = xml;
    return this;
  };
...
Style = (function() {
  ...
  Style.prototype.raw = function(xml) {
    this._raw = xml;
    return this;
  };
...
Workbook = (function() {
  ...
  function Workbook(fpath, fname, opt) {
    var target;
    this.fpath = fpath;
    this.fname = fname;
    this.opt = opt || {};
    this.opt.xlsxTemplatePath =  this.opt.xlsxTemplatePath || path.join(__dirname, 'tmpl');
  ...
  Workbook.prototype.save = function(cb) {
    ...
    async.series([
      async.apply(fs.copy, this.opt.xlsxTemplatePath, target),
      ...
      var zip = new EasyZip();
      zip.zipFolder(target+'/.', function(err){
        if(err) {
          return cb(err);
        }

        zip.writeToFile(path.join(self.fpath, self.fname), function(err) {
          if(err) {
            return cb(err);
          }
...
module.exports = {
  createWorkbook: function(fpath, fname, opt) {    
    return new Workbook(fpath, fname, opt);
  }
};
