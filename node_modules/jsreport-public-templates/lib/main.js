...
PublicTemplates.prototype.generateSharingToken = function (shortid, operation) {
  ...
  return self.reporter.documentStore.collection('templates').find({shortid: shortid}).then(function (templates) {
    ...
    return self.reporter.documentStore.collection('templates').update({shortid: shortid}, {
      $set: {
...
PublicTemplates.prototype.handleAuthorization = function (reporter, definition) {
  if (!reporter.authentication || !reporter.authorization) {
    definition.options.enabled = false
    return
  }

  reporter.initializeListeners.add('public-templates', function () {
    ...
    function addHooksToCollection (col) {
      col.beforeUpdateListeners.insert(0, 'public-templates', col, function (query, update, req) {
        if (!req) {
          return
        }

        if (this.name === 'templates' && !req.user && update.$set && update.$set.writeSharingToken) {
          return verifyToken(req, query, update.$set.writeSharingToken)
        }
      })

      col.beforeInsertListeners.insert(0, 'public-templates', col, function (doc, req) {
        if (!req) {
          return
        }

        if (this.name === 'templatesHistory') {
          req.skipAuthorizationForInsert = doc
        }
      })

      col.beforeFindListeners.add('public-templates', col, function (query, req) {
        if (!req) {
          return
        }
        ...
        var hasToken = req.body.options && req.body.options.authorization && (req.body.options.authorization.writeToken || req.body.options.authorization.readToken)
